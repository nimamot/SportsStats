...
/*-----------------------------------------------------------*/
    ...
    <div class="bavbarMain">
        <div class="title">
            <h1> <a href="hello.php">CPSC 304 SPORTS DATABASE : Soccer</a> </h1>
        </div>

        <div class="navbar">
            <h2 href="#PLAYERS">
                <form method="GET" action="soccer.php">
                    <input type="hidden" id="getAllPlayersRequest" name="getAllPlayersRequest">
                    <input type="submit" name="getAllPlayers" value="PLAYERS" >
                    <input type="hidden" name="sportName" value="Soccer" >
                </form>
            </h2>
              ...
              <div class="dropdown-content">
                ...
                <h3 href="#">Premier League</h3>
                  <ul class='submenu'>  
                    ...
                    <form method="GET" action="soccer.php">
                        ...
                        <input type="hidden" name="leagueName" value="Premier League" >
                    </form>
                    </h2>
                ...
                <div class="has-submenu">
                  ...
                  <h2 href="#Season 2.1">
                    ...
                    <input type="hidden" id="getLeagueRequest" name="getLeagueRequest">
                        <input type="submit" name="getLeague" value="2023" >
                        <input type="hidden" name="leagueName" value="La Liga" >
                    </form>
                    </h2>
    ...
    <div class="dropdown-content">
        <h2>
            <form method="GET" action="soccer.php">
                <input type="hidden" name="getNewsRequest" value="Premier League">
                <input type="submit" name="getNews_1_PremierLeague" value="Premier League">
            </form>
        </h2>
        <h2>
            <form method="GET" action="soccer.php">
                <input type="hidden" name="getNewsRequest" value="La Liga">
                <input type="submit" name="getNews_1_LaLiga" value="La Liga">
            </form>
        </h2>
    </div>
</div>
	...
	// A better coding practice is to have one method that reroutes your requests accordingly. It will make it easier to add/remove functionality.
        function handleGETRequest() {
            if (connectToDB()) {
                ...
                //NEWS
                else if (array_key_exists('getNewsRequest', $_GET)){
                $leagueName = $_GET['getNewsRequest'];
                handleGetNewsRequest($leagueName);
                }
                disconnectFromDB();
            }
        }
        ...
        function handleGetTeamsRequest(){
            ...
            $result = executePlainSQL("SELECT * FROM Teams WHERE Teams.leagueName='La Liga' OR Teams.leagueName='Premier League'");
            ...
            while(true){
                if (($row = oci_fetch_array($result)) != false) {
                    ...
                    echo "<div class='team_object' style='" . $team_object_style . "' >  <img src='". $row[3] ."' width='90'> 
                     <a href=soccer.php?getTeamInfoRequest=&getTeamInfo=" . $team_name_underscore."
        ...
        function handleGetTeamPlayersRequest($teamName){
            ...
            while(true){
                if (($row = oci_fetch_array($result)) != false){
                    ...
                    echo "<a href=soccer.php?getPlayerStatsRequest=&getPlayerStats=". $key ." style='font-size: 15px; margin: 0px; ' > MORE INFO </a>";
                    echo "</ul>";
                    echo "</div>";
                }else break;
            }
        ...
        function handleGetTeamInfoRequest($teamName){
            ...
            if (($row = oci_fetch_array($result)) != false){
                ...
                // echo "<a href=soccer.php?getLeagueRequest=&leagueName=". $row[2] . " 
                // style='font-size:25px; text-decoration:none; margin: 50px;'>  League: "  . $row[2] . " </a>";
                
                echo "<h2 style='margin-left:50px; margin-top:50px; font-family: arial;'> League: " . $row[2] . " </h2>";

                echo "<a href=soccer.php?getTeamPlayersRequest=&getTeamPlayers=". $teamName . " 
                style='font-size:25px; text-decoration:none; margin: 50px;'>  PLAYERS </a>";

                //$temp_1 = str_replace('_', ' ', $teamName);
                ...
                $result_1 = executePlainSQL("SELECT sponsorName FROM sponsoredBy WHERE TeamName='$temp'"); 
                $row_ = oci_fetch_array($result_1);
                echo "<h2 style='margin-left:50px; margin-top:50px; font-family: arial;'> Official Sponsor: " . $row_[0]." </h2>";

                 //style='margin-left:50px; margin-top:50px; font-family: arial;'> Official Sponsor: $row[2] </h2>";
            ...
            // echo "<a href=soccer.php?getTeamStatsRequest=&getTeamStats=" . $teamName ." >  STATISTICS </a>";
        ...
        function handleGetAllPlayersRequest(){
            ...
            while(true){
                if (($row = oci_fetch_array($result)) != false){    
                    ...
                    echo "<h3> <a href=soccer.php?getPlayerStatsRequest=&getPlayerStats=". $key .">" . $row[1]. "</a> </h3>";
        ...
        function handlGetLeagueRequest($season, $leagueName){
            ...
            echo "<h2><a href=soccer.php?getTeamsRequest=&getTeams=TEAMS&leagues=$leagueName> Teams </a><hr /></h2>";
            echo "<h2> MATCHES </h2>";
            ...
            while(true){
                if (($row = oci_fetch_array($matches)) != false){  
                    ...
                    if(($playRow = oci_fetch_array($play)) != false){
                        ...
                        echo "<h3> Home Team: <a href=soccer.php?getTeamInfoRequest=&getTeamInfo=". preg_replace('/\s+/', '_', $playRow[3]) .">".$playRow[3] . 
                        "</a> <br> Away Team: <a href=soccer.php?getTeamInfoRequest=&getTeamInfo=". preg_replace('/\s+/', '_', $playRow[2]) .">".$playRow[2] . "</a></h3>";
        ...
        function handleGetAllCoachesRequest() {
            global $conn;  // Assuming you have a database connection named $conn
        
            // Assuming you have a 'Teams' table for team information
            // Adjust the query based on your actual relationships
            $query = "SELECT c.name_, c.NumOfSeasons, sp.birthDate, sp.nationality, c.teamName
                      FROM coaches c
                      JOIN sportPeople sp ON c.name_ = sp.name_ AND c.birthDate = sp.birthDate AND c.nationality = sp.nationality
                      ORDER BY c.name_";
            
            $result = $conn->query($query);
        
            if ($result->num_rows > 0) {
                echo "<div style='margin-left: 50px; margin-top: 20px;'>";
                // Output data of each row
                while ($row = $result->fetch_assoc()) {
                    echo "<h2>Name: " . $row["name_"] . "</h2>";
                    echo "<p>Number of Seasons: " . $row["NumOfSeasons"] . "</p>";
                    echo "<p>Birth Date: " . $row["birthDate"] . "</p>";
                    echo "<p>Nationality: " . $row["nationality"] . "</p>";
                    echo "<p>Team: " . $row["teamName"] . "</p>";
                    echo "<hr />";
                }
                echo "</div>";
            } else {
        ...
        function handleGetLeagueInfoRequest($leagueName){
            $result = executePlainSQL("SELECT * FROM leagues WHERE leagueName='$leagueName' ");
            
            if(($row = oci_fetch_array($result)) != false){
                echo "<div style='margin-left: 100px; margin-top:50px;'>";
                echo "<img src='" .$row[0]."'>";
                echo "<ul style='font-size:30px;' > Seasons
                <li > <a  href=soccer.php?getLeagueRequest=&getLeague=2023&leagueName=".$leagueName."> 2023 </a> </li>
                <li> <a href=soccer.php?getLeagueRequest=&getLeague=2022&leagueName=".$leagueName."> 2022 </a> </li>
                <li> <a href=soccer.php?getLeagueRequest=&getLeague=2021&leagueName=".$leagueName."> 2021 </a> </li>
                <ul>";
                echo "</div>";
            }   
        ...
        function handleGetNewsRequest($leagueName) {
            global $db_conn;
        
            // Assuming the relationships between tables are as follows:
            // - 'covers' table has foreign keys to 'news_1' for title and author
            // - 'news_1' has a foreign key to 'news_2' for author
        
            // Adjust the query based on your actual relationships
            $query = "SELECT n1.title, n1.author, n1.publishingDate, n1.link
                      FROM news_1 n1
                      JOIN covers c ON n1.title = c.newsTitle AND n1.author = c.newsAuthor
                      WHERE c.leagueName = '$leagueName'";
            ...
            while (($row = oci_fetch_array($result)) != false) {
                ...
                echo "<p>Publishing Date: " . $row['PUBLISHINGDATE'] . "</p>";
